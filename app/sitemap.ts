...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const makeUrl = (path: string): string => {
    ...
    catch { return `${baseUrl}${path.startsWith("/") ? path : `/${path}`}`; }
  };
  ...
  const services = ["residential-cleaning", "commercial-cleaning", "specialized-cleaning"];
  const serviceRoutes: MetadataRoute.Sitemap = services.map((service) => ({ url: makeUrl(`/services/${service}`), changeFrequency: "monthly", priority: 0.7 }));
  ...
  const commercialLandingRoutes: MetadataRoute.Sitemap = commercialLandingPages.map((slug) => ({ url: makeUrl(`/cleaning/${slug}`), changeFrequency: "monthly", priority: 0.9 }));
  ...
  const guideRoutes: MetadataRoute.Sitemap = guides.map((guide) => ({ url: makeUrl(`/guides/${guide}`), changeFrequency: "monthly", priority: 0.6 }));

  let blogRoutes: MetadataRoute.Sitemap = [];
  try {
    const { posts } = await getBlogPosts({ status: "published", limit: 1000 });
    blogRoutes = posts.filter((post) => post.slug && post.slug.trim().length > 0).map((post) => {
      let priority = 0.7;
      ...
      const validDate = dateCandidates.filter(Boolean).map((value) => new Date(value as string)).find((date) => !Number.isNaN(date.getTime()));
      return { url: makeUrl(`/blog/${post.slug}`), ...(validDate ? { lastModified: validDate } : {}), changeFrequency: "weekly" as const, priority };
    });
  } catch (error) { console.error("Error fetching blog posts for sitemap:", error); }

  return [...routes, ...serviceRoutes, ...commercialLandingRoutes, ...locationRoutes, ...guideRoutes, ...blogRoutes];
}
